...
const LastestCard = ({ text }) => {
  const { data, error } = useSWR(
    "http://localhost:1337/articles?_sort=views:DESC&_limit=3",
    fetcher
  );
  const dataTopLastest = useSWR(
    "http://localhost:1337/articles?_sort=public_date:DESC&_limit=5",
    fetcher
  );
  ...
  //     "http://localhost:1337/pages/609a2721a0fa655dac819468",
  //     {
  //       method: "PUT",
  //       headers: {
  ...
  const setCustomContent = (contentEdited) => {
    actions.setCustom(
      selectedNodeId,
      (custom) => (custom.content = contentEdited)
    );
    setProp((props) => ((props.text = "1000"), 500));
  };

  if (!data || !dataTopLastest.data) {
    ...
    <>
      ...
      >
        <EditLastestArticles
          ...
          content={content}
          setContent={(contentEdited) => setContent({ ...contentEdited })}
        />
        <Heading
          marginX=".1em"
          marginTop="1em"
        ...
        <Box d="flex" flexDirection={{ base: "column", lg: "row" }}>
          ...
          >
            {content.featuredLeft === "lastest"
              ? dataTopLastest.data.map((article) => {
                  return (
                    <ArticlesCard
                      data-aos="fade-right"
                      post={article}
